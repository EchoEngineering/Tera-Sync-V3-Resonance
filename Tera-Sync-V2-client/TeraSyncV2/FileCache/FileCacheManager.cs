...
public sealed class FileCacheManager : IHostedService
{
    ...
    public FileCacheManager(ILogger<FileCacheManager> logger, IpcManager ipcManager, TeraSyncConfigService configService, TeraMediator teraMediator)
    {
        _logger = logger;
    ...
    {
        _teraMediator.Publish(new HaltScanMessage(nameof(ValidateLocalIntegrity)));
        ...
        _teraMediator.Publish(new ResumeScanMessage(nameof(ValidateLocalIntegrity)));
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(_csvPath))
        {
            ...
            {
                _teraMediator.Publish(new NotificationMessage("Penumbra not connected",
                    "Could not load local file cache data. Penumbra is not connected or not properly set up. Please enable and/or configure Penumbra properly to use TeraSyncV2. After, reload TeraSyncV2 in the Plugin installer.",
                    TeraSyncConfiguration.Models.NotificationType.Error));
            }
