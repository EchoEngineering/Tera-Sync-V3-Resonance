...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
    ...
    public CacheMonitor(ILogger<CacheMonitor> logger, IpcManager ipcManager, TeraSyncConfigService configService,
        FileCacheManager fileDbManager, TeraMediator mediator, PerformanceCollectorService performanceCollector, DalamudUtilService dalamudUtil,
        FileCompactor fileCompactor) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(this, (_) =>
        {
            ...
            StartTeraWatcher(configService.Current.CacheFolder);
            InvokeScan();
        });
    ...
    private readonly Dictionary<string, WatcherChange> _teraChanges = new Dictionary<string, WatcherChange>(StringComparer.OrdinalIgnoreCase);
    ...
    public void StartTeraWatcher(string? teraPath)
    {
        TeraWatcher?.Dispose();
        if (string.IsNullOrEmpty(teraPath) || !Directory.Exists(teraPath))
        {
            TeraWatcher = null;
            Logger.LogWarning("TeraSyncV2 file path is not set, cannot start the FSW for Tera Sync V2.");
            return;
        }
        ...
        Logger.LogInformation("TeraSyncV2 Storage is on NTFS drive: {isNtfs}", StorageisNTFS);

        Logger.LogDebug("Initializing TeraSyncV2 FSW on {path}", teraPath);
        TeraWatcher = new()
        {
            Path = teraPath,
        ...
        TeraWatcher.Deleted += TeraWatcher_FileChanged;
        TeraWatcher.Created += TeraWatcher_FileChanged;
        TeraWatcher.EnableRaisingEvents = true;
    }

    private void TeraWatcher_FileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("TeraSyncV2 FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);

        if (!AllowedFileExtensions.Any(ext => e.FullPath.EndsWith(ext, StringComparison.OrdinalIgnoreCase))) return;

        lock (_watcherChanges)
        {
            _teraChanges[e.FullPath] = new(e.ChangeType);
        }

        _ = TeraWatcherExecution();
    }
    ...
    private CancellationTokenSource _teraFswCts = new();
    public FileSystemWatcher? PenumbraWatcher { get; private set; }
    public FileSystemWatcher? TeraWatcher { get; private set; }

    private async Task TeraWatcherExecution()
    {
        _teraFswCts = _teraFswCts.CancelRecreate();
        var token = _teraFswCts.Token;
        var delay = TimeSpan.FromSeconds(5);
        Dictionary<string, WatcherChange> changes;
        lock (_teraChanges)
            changes = _teraChanges.ToDictionary(t => t.Key, t => t.Value, StringComparer.Ordinal);
        try
        {
            do
            {
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("TeraSyncV2 Cache directory is not set or does not exist.");
        }
        ...
        if (!_configService.Current.InitialScanComplete)
        {
            ...
            _configService.Save();
            StartTeraWatcher(_configService.Current.CacheFolder);
