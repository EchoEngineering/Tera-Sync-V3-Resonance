...
public class FileTransferOrchestrator : DisposableMediatorSubscriberBase
{
    ...
    private readonly TeraSyncConfigService _teraConfig;
    ...
    public FileTransferOrchestrator(ILogger<FileTransferOrchestrator> logger, TeraSyncConfigService teraConfig,
        TeraMediator mediator, TokenProvider tokenProvider, HttpClient httpClient) : base(logger, mediator)
    {
        _teraConfig = teraConfig;
        _tokenProvider = tokenProvider;
        _httpClient = httpClient;
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("TeraSyncV2", ver!.Major + "." + ver!.Minor + "." + ver!.Build));

        _availableDownloadSlots = teraConfig.Current.ParallelDownloads;
    ...
    public async Task WaitForDownloadSlotAsync(CancellationToken token)
    {
        ...
        {
            if (_availableDownloadSlots != _teraConfig.Current.ParallelDownloads && _availableDownloadSlots == _downloadSemaphore.CurrentCount)
            {
                _availableDownloadSlots = _teraConfig.Current.ParallelDownloads;
                _downloadSemaphore = new(_availableDownloadSlots, _availableDownloadSlots);
            }
        }
    ...
    {
        var limit = _teraConfig.Current.DownloadSpeedLimitInBytes;
        if (limit <= 0) return 0;
        limit = _teraConfig.Current.DownloadSpeedType switch
        {
            TeraSyncConfiguration.Models.DownloadSpeeds.Bps => limit,
            TeraSyncConfiguration.Models.DownloadSpeeds.KBps => limit * 1024,
            TeraSyncConfiguration.Models.DownloadSpeeds.MBps => limit * 1024 * 1024,
            _ => limit,
        };
