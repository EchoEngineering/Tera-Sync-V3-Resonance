...
public sealed partial class ApiController : DisposableMediatorSubscriberBase, ITeraHubClient
{
    ...
    private HubConnection? _teraHub;
    private ServerState _serverState;
    private CensusUpdateMessage? _lastCensus;

    public ApiController(ILogger<ApiController> logger, HubFactory hubFactory, DalamudUtilService dalamudUtil,
        PairManager pairManager, ServerConfigurationManager serverManager, TeraMediator mediator,
        TokenProvider tokenProvider, TeraSyncConfigService teraConfigService) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<HubClosedMessage>(this, (msg) => TeraHubOnClosed(msg.Exception));
        Mediator.Subscribe<HubReconnectedMessage>(this, (msg) => _ = TeraHubOnReconnectedAsync());
        Mediator.Subscribe<HubReconnectingMessage>(this, (msg) => TeraHubOnReconnecting(msg.Exception));
    ...
    {
        return await _teraHub!.InvokeAsync<bool>(nameof(CheckClientHealth)).ConfigureAwait(false);
    }
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to Tera Sync V2.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                catch (TeraAuthFailureException ex)
                {
                ...
                _teraHub = _hubFactory.GetOrCreate(token);
                InitializeApiHooks();

                await _teraHub.StartAsync(token).ConfigureAwait(false);
                ...
                if (_connectionDto.ServerVersion != ITeraHub.ApiVersion)
                {
                    if (_connectionDto.CurrentClientVersion > currentClientVer)
                    {
                        Mediator.Publish(new NotificationMessage("Client incompatible",
                            ...
                            $"This client version is incompatible and will not be able to connect. Please update your Tera Sync V2 client.",
                            NotificationType.Error));
                    }
                ...
                if (_connectionDto.CurrentClientVersion > currentClientVer)
                {
                    Mediator.Publish(new NotificationMessage("Client outdated",
                        $"Your client is outdated ({currentClientVer.Major}.{currentClientVer.Minor}.{currentClientVer.Build}), current is: " +
                        $"{_connectionDto.CurrentClientVersion.Major}.{_connectionDto.CurrentClientVersion.Minor}.{_connectionDto.CurrentClientVersion.Build}. " +
                        $"Please keep your Tera Sync V2 client up-to-date.",
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_teraSyncConfigService.Current.DebugStopWhining)
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "Tera Sync V2, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
                ...
                if (_dalamudUtil.IsLodEnabled && !_naggedAboutLod)
                {
                    ...
                    if (!_teraSyncConfigService.Current.DebugStopWhining)
                    {
                        Mediator.Publish(new NotificationMessage("Model LOD is enabled",
                            ...
                            "random crashes when loading in or rendering modded pairs. Disabling LOD has a very low performance impact. Disable LOD while using Tera Sync V2: " +
    ...
    {
        var dto = await _teraHub!.InvokeAsync<ConnectionDto>(nameof(GetConnectionDto)).ConfigureAwait(false);
    ...
    {
        while (!ct.IsCancellationRequested && _teraHub != null)
        {
    ...
    private void TeraHubOnClosed(Exception? arg)
    {
        _healthCheckTokenSource?.Cancel();
    ...
    private async Task TeraHubOnReconnectedAsync()
    {
        ServerState = ServerState.Reconnecting;
        try
        {
    ...
    private void TeraHubOnReconnecting(Exception? arg)
    {
