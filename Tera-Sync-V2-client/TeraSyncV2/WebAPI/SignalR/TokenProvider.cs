...
public sealed class TokenProvider : IDisposable, IMediatorSubscriber
{
    ...
    public TokenProvider(ILogger<TokenProvider> logger, ServerConfigurationManager serverManager, DalamudUtilService dalamudUtil, TeraMediator teraMediator, HttpClient httpClient)
    {
        _logger = logger;
        _serverManager = serverManager;
        _dalamudUtil = dalamudUtil;
        Mediator = teraMediator;
    ...
    public async Task<string> GetNewToken(bool isRenewal, JwtIdentifier identifier, CancellationToken ct)
    {
        ...
        try
        {
            if (!isRenewal)
            {
                ...
                {
                    tokenUri = TeraAuth.AuthFullPath(new Uri(_serverManager.CurrentApiUrl
                ...
                {
                    tokenUri = TeraAuth.AuthWithOauthFullPath(new Uri(_serverManager.CurrentApiUrl
                        .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
                        .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
            ...
            else
            {
                ...
                tokenUri = TeraAuth.RenewTokenFullPath(new Uri(_serverManager.CurrentApiUrl
                    .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
                    .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
        ...
        catch (HttpRequestException ex)
        {
            ...
            if (ex.StatusCode == System.Net.HttpStatusCode.Unauthorized)
            {
                ...
                throw new TeraAuthFailureException(response);
            }
        ...
        if (tokenTime <= dateTimeMinus10 || tokenTime >= dateTimePlus10)
        {
            ...
            Mediator.Publish(new NotificationMessage("Invalid system clock", "The clock of your computer is invalid. " +
                "Tera Sync V2 will not function properly if the time zone is not set correctly. " +
    ...
    public async Task<bool> TryUpdateOAuth2LoginTokenAsync(ServerStorage currentServer, bool forced = false)
    {
        ...
        var tokenUri = TeraAuth.RenewOAuthTokenFullPath(new Uri(currentServer.ServerUri
            .Replace("wss://", "https://", StringComparison.OrdinalIgnoreCase)
            .Replace("ws://", "http://", StringComparison.OrdinalIgnoreCase)));
        HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, tokenUri.ToString());
