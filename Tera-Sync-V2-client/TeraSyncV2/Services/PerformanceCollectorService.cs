...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, TeraSyncConfigService teraConfigService)
    {
        _logger = logger;
        _teraSyncConfigService = teraConfigService;
    }

    public T LogPerformance<T>(object sender, TeraInterpolatedStringHandler counterName, Func<T> func, int maxEntries = 10000)
    {
        if (!_teraSyncConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, TeraInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_teraSyncConfigService.Current.LogPerformance) { act.Invoke(); return; }
    ...
    {
        if (!_teraSyncConfigService.Current.LogPerformance)
        {
