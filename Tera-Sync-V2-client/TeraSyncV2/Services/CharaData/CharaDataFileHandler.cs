...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly TeraCharaFileDataFactory _teraCharaFileDataFactory;
            ...
            DalamudUtilService dalamudUtilService, GameObjectHandlerFactory gameObjectHandlerFactory, PlayerDataFactory playerDataFactory)
    {
        ...
        _teraCharaFileDataFactory = new(fileCacheManager);
    }
    ...
    public Task<(TeraCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = TeraCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Tera Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(TeraCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        TeraCharaFileHeader.AdvanceReaderToData(reader);

        long totalRead = 0;
        Dictionary<string, string> gamePathToFilePath = new(StringComparer.Ordinal);
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(_fileCacheManager.CacheFolder, "tera_" + _globalFileCounter++ + ".tmp");
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var teraCharaFileData = _teraCharaFileDataFactory.Create(description, data);
            TeraCharaFileHeader output = new(TeraCharaFileHeader.CurrentVersion, teraCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving Tera Chara File, deleting output");
