...
public class IdDisplayHandler
{
    ...
    public IdDisplayHandler(TeraMediator mediator, ServerConfigurationManager serverManager, TeraSyncConfigService teraConfigService)
    {
        _mediator = mediator;
    ...
    public void DrawPairText(string id, Pair pair, float textPosX, Func<float> editBoxWidth)
    {
        ...
        if (!string.Equals(_editEntry, pair.UserData.UID, StringComparison.Ordinal))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                {
                    _popupTime = DateTime.UtcNow.AddSeconds(_teraSyncConfigService.Current.ProfileDelay);
                }

                _lastMouseOverUid = id;

                if (_popupTime > DateTime.UtcNow || !_teraSyncConfigService.Current.ProfilesShow)
                {
    ...
    public (bool isUid, string text) GetPlayerText(Pair pair)
    {
        ...
        if (_teraSyncConfigService.Current.ShowCharacterNameInsteadOfNotesForVisible && pair.IsVisible && !showUidInsteadOfName)
        {
            playerText = pair.PlayerName;
            textIsUid = false;
            if (_teraSyncConfigService.Current.PreferNotesOverNamesForVisible)
            {
