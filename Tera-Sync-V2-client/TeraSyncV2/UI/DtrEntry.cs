...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private readonly ConfigurationServiceBase<TeraSyncConfig> _configService;
    ...
    public DtrEntry(ILogger<DtrEntry> logger, IDtrBar dtrBar, ConfigurationServiceBase<TeraSyncConfig> configService, TeraMediator teraMediator, PairManager pairManager, ApiController apiController)
    {
        _logger = logger;
        _dtrBar = dtrBar;
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("TeraSyncV2");
        entry.OnClick = _ => _teraMediator.Publish(new UiToggleMessage(typeof(CompactUi)));
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"Tera Sync V2: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "Tera Sync V2: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "Tera Sync V2: Not Connected";
