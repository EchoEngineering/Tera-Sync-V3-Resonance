...
public class CompactUi : WindowMediatorSubscriberBase
{
    ...
    public CompactUi(ILogger<CompactUi> logger, UiSharedService uiShared, TeraSyncConfigService configService, ApiController apiController, PairManager pairManager,
        ServerConfigurationManager serverManager, TeraMediator mediator, FileUploadManager fileTransferManager,
        TagHandler tagHandler, DrawEntityFactory drawEntityFactory, SelectTagForPairUi selectTagForPairUi, SelectPairForTagUi selectPairForTagUi,
        PerformanceCollectorService performanceCollectorService, IpcManager ipcManager)
        : base(logger, mediator, "###TeraSyncMainUI", performanceCollectorService)
    {
        ...
        TitleBarButtons = new()
        {
            new Window.TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Tera Sync V2 Settings");
                    ImGui.EndTooltip();
                }
            },
            new Window.TitleBarButton()
            {
                ...
                {
                    ImGui.BeginTooltip();
                    ImGui.Text("Open Tera Sync V2 Event Viewer");
                    ImGui.EndTooltip();
                }
            }
        };
        ...
        WindowName = $"Tera Sync V2 {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###TeraSyncMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Tera Sync V2 " + ver.Major + "." + ver.Minor + "." + ver.Build + "###TeraSyncMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Tera Sync V2 installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Tera Sync V2 up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
        ...
        if (!_ipcManager.Initialized)
        {
            ...
            UiSharedService.ColorTextWrapped($"One or more Plugins essential for Tera Sync V2 operation are unavailable. Enable or update following plugins:", ImGuiColors.DalamudRed);
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Tera Sync server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Tera Sync server is currently offline.",
            ...
            ServerState.NoAutoLogon => "This character has automatic login into Tera Sync V2 disabled. Press the connect button to connect to Tera Sync V2.",
            _ => string.Empty
        };
    }
