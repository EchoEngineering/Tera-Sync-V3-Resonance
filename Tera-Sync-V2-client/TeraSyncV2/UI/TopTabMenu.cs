...
public class TopTabMenu
{
    ...
    public TopTabMenu(TeraMediator teraMediator, ApiController apiController, PairManager pairManager, UiSharedService uiSharedService)
    {
    ...
    public string Filter
    {
        ...
        private set
        {
            ...
            {
                _teraMediator.Publish(new RefreshUiMessage());
            }
    ...
    private void DrawSyncshellMenu(float availableWidth, float spacingX)
    {
            ...
            .Count(g => string.Equals(g.OwnerUID, _apiController.UID, StringComparison.Ordinal)) >= _apiController.ServerInfo.MaxGroupsCreatedByUser))
        {
            ...
            {
                _teraMediator.Publish(new UiToggleMessage(typeof(CreateSyncshellUI)));
            }
            ImGui.SameLine();
        }
        ...
        using (ImRaii.Disabled(_pairManager.GroupPairs.Select(k => k.Key).Distinct().Count() >= _apiController.ServerInfo.MaxGroupsJoinedByUser))
        {
            ...
            {
                _teraMediator.Publish(new UiToggleMessage(typeof(JoinSyncshellUI)));
            }
        }
    }
    ...
    private void DrawUserConfig(float availableWidth, float spacingX)
    {
        ...
        UiSharedService.AttachToolTip("Edit your Tera Sync V2 Profile");
        ImGui.SameLine();
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.PersonCircleQuestion, "Chara Data Analysis", buttonX))
        {
            _teraMediator.Publish(new UiToggleMessage(typeof(DataAnalysisUi)));
        }
        ...
        {
            _teraMediator.Publish(new UiToggleMessage(typeof(CharaDataHubUi)));
        }
    }
