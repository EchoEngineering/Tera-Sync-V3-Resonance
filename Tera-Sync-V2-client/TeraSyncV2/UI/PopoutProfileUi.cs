...
public class PopoutProfileUi : WindowMediatorSubscriberBase
{
    ...
    public PopoutProfileUi(ILogger<PopoutProfileUi> logger, TeraMediator mediator, UiSharedService uiBuilder,
        ServerConfigurationManager serverManager, TeraSyncConfigService teraConfigService,
        TeraProfileManager teraProfileManager, PairManager pairManager, PerformanceCollectorService performanceCollectorService) : base(logger, mediator, "###TeraSyncPopoutProfileUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<CompactUiChange>(this, (msg) =>
        {
            ...
            if (teraConfigService.Current.ProfilePopoutRight)
            {
    ...
    protected override void DrawInternal()
    {
        ...
        try
        {
            ...
            var teraProfile = _teraProfileManager.GetTeraProfile(_pair.UserData);

            if (_textureWrap == null || !teraProfile.ImageData.Value.SequenceEqual(_lastProfilePicture))
            {
                _textureWrap?.Dispose();
                _lastProfilePicture = teraProfile.ImageData.Value;
                _textureWrap = _uiSharedService.LoadImage(_lastProfilePicture);
            }

            if (_supporterTextureWrap == null || !teraProfile.SupporterImageData.Value.SequenceEqual(_lastSupporterPicture))
            {
                _supporterTextureWrap?.Dispose();
                _supporterTextureWrap = null;
                if (!string.IsNullOrEmpty(teraProfile.Base64SupporterPicture))
                {
                    _lastSupporterPicture = teraProfile.SupporterImageData.Value;
            ...
            var descText = teraProfile.Description;
            ...
            UiSharedService.TextWrapped(trimmed ? descText + $"...{Environment.NewLine}[Open Full Profile for complete description]" : teraProfile.Description);
            font.Dispose();
