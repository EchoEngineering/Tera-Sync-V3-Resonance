...
public sealed class IpcCallerGlamourer : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public IpcCallerGlamourer(ILogger<IpcCallerGlamourer> logger, IDalamudPluginInterface pi, DalamudUtilService dalamudUtil, TeraMediator teraMediator,
        RedrawManager redrawManager) : base(logger, teraMediator)
    {
    ...
    public void CheckAPI()
    {
        ...
        finally
        {
            if (!apiAvailable && !_shownGlamourerUnavailable)
            {
                ...
                _teraMediator.Publish(new NotificationMessage("Glamourer inactive", "Your Glamourer installation is not active or out of date. Update Glamourer to continue to use Tera Sync V2. If you just updated Glamourer, ignore this message.",
        ...
        try
        {
            ...
            await _redrawManager.PenumbraRedrawInternalAsync(logger, handler, applicationId, (chara) =>
            {
                try
                {
                    ...
                    _teraMediator.Publish(new PenumbraRedrawCharacterMessage(chara));
                }
                catch (Exception ex)
                {
    ...
    {
        _teraMediator.Publish(new GlamourerChangedMessage(address));
    }
}
