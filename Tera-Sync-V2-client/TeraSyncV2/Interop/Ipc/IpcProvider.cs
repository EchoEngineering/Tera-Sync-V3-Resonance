...
public class IpcProvider : IHostedService, IMediatorSubscriber
{
    ...
    public TeraMediator Mediator { get; init; }

    public IpcProvider(ILogger<IpcProvider> logger, IDalamudPluginInterface pi,
        CharaDataManager charaDataManager, TeraMediator teraMediator)
    {
        _logger = logger;
        _pi = pi;
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _loadFileProvider = _pi.GetIpcProvider<string, IGameObject, bool>("TeraSyncV2.LoadMcdf");
        _loadFileProvider.RegisterFunc(LoadMcdf);
        _loadFileAsyncProvider = _pi.GetIpcProvider<string, IGameObject, Task<bool>>("TeraSyncV2.LoadMcdfAsync");
        _loadFileAsyncProvider.RegisterFunc(LoadMcdfAsync);
        _handledGameAddresses = _pi.GetIpcProvider<List<nint>>("TeraSyncV2.GetHandledAddresses");
