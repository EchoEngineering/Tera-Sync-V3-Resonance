...
public sealed class IpcCallerPenumbra : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public string? ModDirectory
    {
        ...
        private set
        {
            if (!string.Equals(_penumbraModDirectory, value, StringComparison.Ordinal))
            {
                ...
                _teraMediator.Publish(new PenumbraDirectoryChangedMessage(_penumbraModDirectory));
            }
        }
    }
    ...
    public IpcCallerPenumbra(ILogger<IpcCallerPenumbra> logger, IDalamudPluginInterface pi, DalamudUtilService dalamudUtil,
        TeraMediator teraMediator, RedrawManager redrawManager) : base(logger, teraMediator)
    {
        _pi = pi;
        ...
        _penumbraModSettingChanged = ModSettingChanged.Subscriber(pi, (change, arg1, arg, b) =>
        {
            if (change == ModSettingChange.EnableState)
                _teraMediator.Publish(new PenumbraModSettingChangedMessage());
        });
    ...
    public async Task ConvertTextureFiles(ILogger logger, Dictionary<string, string[]> textures, IProgress<(string, int)> progress, CancellationToken token)
    {
        ...
        _teraMediator.Publish(new HaltScanMessage(nameof(ConvertTextureFiles)));
        ...
        _teraMediator.Publish(new ResumeScanMessage(nameof(ConvertTextureFiles)));
    ...
    public async Task<Guid> CreateTemporaryCollectionAsync(ILogger logger, string uid)
    {
        ...
        {
            var collName = "Tera_" + uid;
            _penumbraCreateNamedTemporaryCollection.Invoke("TeraSyncV2", collName, out Guid collId);
    ...
    public async Task SetManipulationDataAsync(ILogger logger, Guid applicationId, Guid collId, string manipulationData)
    {
        ...
        await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            var retAdd = _penumbraAddTemporaryMod.Invoke("TeraChara_Meta", collId, [], manipulationData, 0);
    ...
    public async Task SetTemporaryModsAsync(ILogger logger, Guid applicationId, Guid collId, Dictionary<string, string> modPaths)
    {
        ...
        await _dalamudUtil.RunOnFrameworkThread(() =>
        {
            ...
            var retRemove = _penumbraRemoveTemporaryMod.Invoke("TeraChara_Files", collId, 0);
            logger.LogTrace("[{applicationId}] Removing temp files mod for {collId}, Success: {ret}", applicationId, collId, retRemove);
            var retAdd = _penumbraAddTemporaryMod.Invoke("TeraChara_Files", collId, modPaths, string.Empty, 0);
    ...
    private void RedrawEvent(IntPtr objectAddress, int objectTableIndex)
    {
        ...
        {
            _teraMediator.Publish(new PenumbraRedrawMessage(objectAddress, objectTableIndex, wasRequested));
        }
    }
    ...
    private void ResourceLoaded(IntPtr ptr, string arg1, string arg2)
    {
        ...
        {
            _teraMediator.Publish(new PenumbraResourceLoadMessage(ptr, arg1, arg2));
        }
    }
    ...
    {
        _redrawManager.Cancel();
        _teraMediator.Publish(new PenumbraDisposedMessage());
    }
    ...
    private void PenumbraInit()
    {
        ...
        _teraMediator.Publish(new PenumbraInitializedMessage());
