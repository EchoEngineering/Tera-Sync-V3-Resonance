...
namespace TeraSyncV2ServerTest.Hubs {
    public class TeraHubTest {

        [Test]
        public async Task Disconnect_QueryReturnsCorrectResult_Test() {
            var options = new DbContextOptionsBuilder<TeraDbContext>()
                .UseInMemoryDatabase(databaseName: "tera").Options;

            using var context = new TeraDbContext(options);
            ...
            var hub = new TeraHub(context, new Mock<ILogger<TeraHub>>().Object, null, new Mock<IConfiguration>().Object, new Mock<IHttpContextAccessor>().Object);
