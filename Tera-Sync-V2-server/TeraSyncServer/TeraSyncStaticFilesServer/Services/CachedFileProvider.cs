...
public sealed class CachedFileProvider : IDisposable
{
    ...
    public CachedFileProvider(IConfigurationService<StaticFilesServerConfiguration> configuration, ILogger<CachedFileProvider> logger,
        FileStatisticsService fileStatisticsService, TeraMetrics metrics, ServerTokenGenerator generator)
    {
        ...
        _httpClient = new();
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("TeraSyncV2Server", "1.0.0.0"));
    ...
    private async Task DownloadTask(string hash)
    {
        ...
        var downloadUrl = TeraFiles.DistributionGetFullPath(_remoteCacheSourceUri, hash);
