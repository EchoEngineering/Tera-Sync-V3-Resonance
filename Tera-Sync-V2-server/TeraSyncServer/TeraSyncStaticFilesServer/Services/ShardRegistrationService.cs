...
public class ShardRegistrationService : IHostedService
{
    ...
    private async Task ShardHeartbeat(CancellationToken ct)
    {
        ...
        using var heartBeat = await _httpClient.PostAsync(new Uri(mainServer, TeraFiles.Main + "/shardHeartbeat"), null, ct).ConfigureAwait(false);
    ...
    private async Task TryRegisterShard(CancellationToken ct)
    {
        ...
        using var register = await _httpClient.PostAsJsonAsync(new Uri(mainServer, TeraFiles.Main + "/shardRegister"), config, ct).ConfigureAwait(false);
    ...
    private async Task UnregisterShard()
    {
        ...
        using var heartBeat = await _httpClient.PostAsync(new Uri(mainServer, TeraFiles.Main + "/shardUnregister"), null).ConfigureAwait(false);
    }
}
