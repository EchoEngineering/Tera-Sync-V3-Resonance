...
using TeraSyncV2StaticFilesServer.Controllers;
...
namespace TeraSyncV2StaticFilesServer;
...
public class Startup
{
    ...
    public Startup(IConfiguration configuration, ILogger<Startup> logger)
    {
        ...
        _logger = logger;
        var teraSettings = Configuration.GetRequiredSection("TeraSyncV2");
        _isDistributionNode = teraSettings.GetValue(nameof(StaticFilesServerConfiguration.IsDistributionNode), false);
        _isMain = string.IsNullOrEmpty(teraSettings.GetValue(nameof(StaticFilesServerConfiguration.MainFileServerAddress), string.Empty)) && _isDistributionNode;
    }
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        services.Configure<StaticFilesServerConfiguration>(Configuration.GetRequiredSection("TeraSyncV2"));
        ...
        if (_isMain)
        {
            ...
            services.AddSingleton<IConfigurationService<StaticFilesServerConfiguration>, TeraConfigurationServiceServer<StaticFilesServerConfiguration>>();
        ...
        else
        {
            ...
            services.AddSingleton<IConfigurationService<StaticFilesServerConfiguration>, TeraConfigurationServiceClient<StaticFilesServerConfiguration>>();
            services.AddHostedService(p => (TeraConfigurationServiceClient<StaticFilesServerConfiguration>)p.GetService<IConfigurationService<StaticFilesServerConfiguration>>());
        }
        ...
        services.AddControllers().ConfigureApplicationPartManager(a =>
        {
            ...
            if (_isMain)
            {
                a.FeatureProviders.Add(new AllowedControllersFeatureProvider(typeof(TeraStaticFilesServerConfigurationController),
        ...
        services.AddOptions<JwtBearerOptions>(JwtBearerDefaults.AuthenticationScheme)
            .Configure<IConfigurationService<TeraSyncConfigurationBase>>((o, s) =>
            {
                o.TokenValidationParameters = new()
                {
                    ...
                    IssuerSigningKey = new SymmetricSecurityKey(Encoding.ASCII.GetBytes(s.GetValue<string>(nameof(TeraSyncConfigurationBase.Jwt))))
                };
            });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
    {
        ...
        var metricServer = new KestrelMetricServer(config.GetValueOrDefault<int>(nameof(TeraSyncConfigurationBase.MetricsPort), 4981));
        metricServer.Start();
        ...
        app.UseEndpoints(e =>
        {
            if (_isMain)
            {
                e.MapHub<TeraSyncV2Server.Hubs.TeraHub>("/dummyhub");
            }
