...
public class ServerFilesController : ControllerBase
{
    ...
    private readonly IHubContext<TeraHub> _hubContext;
    private readonly IDbContextFactory<TeraDbContext> _teraDbContext;
    private readonly TeraMetrics _metricsClient;
    ...
    public ServerFilesController(ILogger<ServerFilesController> logger, CachedFileProvider cachedFileProvider,
        ...
        IHubContext<TeraHub> hubContext,
        IDbContextFactory<TeraDbContext> teraDbContext, TeraMetrics metricsClient,
    ...
    [HttpPost(TeraFiles.ServerFiles_DeleteAll)]
    public async Task<IActionResult> FilesDeleteAll()
    {
        using var dbContext = await _teraDbContext.CreateDbContextAsync();
        var ownFiles = await dbContext.Files.Where(f => f.Uploaded && f.Uploader.UID == TeraUser).ToListAsync().ConfigureAwait(false);
    ...
    [HttpGet(TeraFiles.ServerFiles_GetSizes)]
    ...
    [HttpGet(TeraFiles.ServerFiles_DownloadServers)]
    ...
    [HttpPost(TeraFiles.ServerFiles_FilesSend)]
    public async Task<IActionResult> FilesSend([FromBody] FilesSendDto filesSendDto)
    {
        ...
        {
            await _hubContext.Clients.Users(filesSendDto.UIDs).SendAsync(nameof(ITeraHub.Client_UserReceiveUploadStatus), new TeraSyncV2.API.Dto.User.UserDto(new(TeraUser)))
    ...
    [HttpPost(TeraFiles.ServerFiles_Upload + "/{hash}")]
    [RequestSizeLimit(200 * 1024 * 1024)]
    public async Task<IActionResult> UploadFile(string hash, CancellationToken requestAborted)
    {
        using var dbContext = await _teraDbContext.CreateDbContextAsync();

        _logger.LogInformation("{user}|{file}: Uploading", TeraUser, hash);
        ...
        try
        {
            ...
            _logger.LogDebug("{user}|{file}: Finished uploading", TeraUser, hash);
        ...
        {
            _logger.LogError(e, "{user}|{file}: Error during file upload", TeraUser, hash);
            return BadRequest();
        }
        finally
        {
    ...
    [HttpPost(TeraFiles.ServerFiles_UploadMunged + "/{hash}")]
    [RequestSizeLimit(200 * 1024 * 1024)]
    public async Task<IActionResult> UploadFileMunged(string hash, CancellationToken requestAborted)
    {
        using var dbContext = await _teraDbContext.CreateDbContextAsync();

        _logger.LogInformation("{user}|{file}: Uploading munged", TeraUser, hash);
        hash = hash.ToUpperInvariant();
        var existingFile = await dbContext.Files.SingleOrDefaultAsync(f => f.Hash == hash);
        if (existingFile != null) return Ok();
        ...
        try
        {
            ...
            _logger.LogDebug("{user}|{file}: Finished uploading, unmunged stream", TeraUser, hash);
    ...
    private async Task StoreData(string hash, TeraDbContext dbContext, MemoryStream compressedFileStream)
    {
        ...
        if (!string.Equals(hashString, hash, StringComparison.Ordinal))
            throw new InvalidOperationException($"{TeraUser}|{hash}: Hash does not match file, computed: {hashString}, expected: {hash}");
        ...
        _logger.LogDebug("{user}|{file}: Uploaded file saved to {path}", TeraUser, hash, path);
        ...
        _logger.LogDebug("{user}|{file}: Uploaded file saved to DB", TeraUser, hash);
    ...
    private async Task<SemaphoreSlim> CreateFileLock(string hash, CancellationToken requestAborted)
    {
        ...
        while (!successfullyWaited && !requestAborted.IsCancellationRequested)
        {
            lock (_fileUploadLocks)
            {
                ...
                {
                    _logger.LogDebug("{user}|{file}: Creating filelock", TeraUser, hash);
            ...
            try
            {
                _logger.LogDebug("{user}|{file}: Waiting for filelock", TeraUser, hash);
                await fileLock.WaitAsync(requestAborted).ConfigureAwait(false);
                successfullyWaited = true;
            }
            catch (ObjectDisposedException)
            {
                _logger.LogWarning("{user}|{file}: Semaphore disposed, recreating", TeraUser, hash);
            }
        }
