...
public class TeraConfigurationServiceClient<T> : IHostedService, IConfigurationService<T> where T : class, ITeraConfiguration
{
    ...
    private readonly ILogger<TeraConfigurationServiceClient<T>> _logger;
    ...
    private Uri GetRoute(string key, string value)
    {
        if (_config.CurrentValue.GetType() == typeof(ServerConfiguration))
            return new Uri((_config.CurrentValue as ServerConfiguration).MainServerAddress, $"configuration/TeraServerConfiguration/{nameof(TeraServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(TeraSyncConfigurationBase))
            return new Uri((_config.CurrentValue as TeraSyncConfigurationBase).MainServerAddress, $"configuration/TeraBaseConfiguration/{nameof(TeraBaseConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(ServicesConfiguration))
            return new Uri((_config.CurrentValue as ServicesConfiguration).MainServerAddress, $"configuration/TeraServicesConfiguration/{nameof(TeraServicesConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");
        if (_config.CurrentValue.GetType() == typeof(StaticFilesServerConfiguration))
            return new Uri((_config.CurrentValue as StaticFilesServerConfiguration).MainFileServerAddress, $"configuration/TeraStaticFilesServerConfiguration/{nameof(TeraStaticFilesServerConfigurationController.GetConfigurationEntry)}?key={key}&defaultValue={value}");

        throw new NotSupportedException("Config is not supported to be gotten remotely");
    }

    public TeraConfigurationServiceClient(ILogger<TeraConfigurationServiceClient<T>> logger, IOptionsMonitor<T> config, ServerTokenGenerator serverTokenGenerator)
    {
        _config = config;
        _logger = logger;
    ...
    {
        _logger.LogInformation("Starting TeraConfigurationServiceClient");
