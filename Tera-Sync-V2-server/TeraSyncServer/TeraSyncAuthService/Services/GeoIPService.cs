...
namespace TeraSyncV2AuthService.Services;

public class GeoIPService : IHostedService
{
    private readonly ILogger<GeoIPService> _logger;
    private readonly IConfigurationService<AuthServiceConfiguration> _teraConfiguration;
    ...
    public GeoIPService(ILogger<GeoIPService> logger,
        IConfigurationService<AuthServiceConfiguration> teraConfiguration)
    {
        _logger = logger;
        _teraConfiguration = teraConfiguration;
    }
    ...
    private async Task PeriodicReloadTask(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            try
            {
                ...
                var useGeoIP = _teraConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.UseGeoIP), false);
                var cityFile = _teraConfiguration.GetValueOrDefault(nameof(AuthServiceConfiguration.GeoIPDbCityFile), string.Empty);
