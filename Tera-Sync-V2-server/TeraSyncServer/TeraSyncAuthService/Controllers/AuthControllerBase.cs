using TeraSyncV2AuthService.Authentication;
...
public abstract class AuthControllerBase : Controller
{
    ...
    protected readonly IDbContextFactory<TeraDbContext> TeraDbContextFactory;
    ...
    IDatabase redisDb, GeoIPService geoIPProvider)
    {
        ...
        TeraDbContextFactory = teraDbContextFactory;
        SecretKeyAuthenticatorService = secretKeyAuthenticatorService;
        Configuration = configuration;
    }

    protected async Task<IActionResult> GenericAuthResponse(TeraDbContext dbContext, string charaIdent, SecretKeyAuthReply authResult)
    {
        ...
        if (!authResult.Success && !authResult.TempBan)
        {
            ...
            return Unauthorized("The provided secret key is invalid. Verify your Tera Sync V2 accounts existence and/or recover the secret key.");
        }
        if (!authResult.Success && authResult.TempBan)
        {
        ...
        if (authResult.Permaban || authResult.MarkedForBan)
        {
            ...
            return Unauthorized("Your Tera Sync V2 account is banned from using the service.");
        }
        ...
        if (!string.IsNullOrEmpty(existingIdent))
        {
            ...
            return Unauthorized("Already logged in to this Tera Sync V2 account. Reconnect in 60 seconds. If you keep seeing this issue, restart your game.");
        }
    ...
    {
        var authSigningKey = new SymmetricSecurityKey(Encoding.ASCII.GetBytes(Configuration.GetValue<string>(nameof(TeraSyncConfigurationBase.Jwt))));

        var token = new SecurityTokenDescriptor()
        {
            Subject = new ClaimsIdentity(authClaims),
            SigningCredentials = new SigningCredentials(authSigningKey, SecurityAlgorithms.HmacSha256Signature),
            Expires = new(long.Parse(authClaims.First(f => string.Equals(f.Type, TeraClaimTypes.Expires, StringComparison.Ordinal)).Value!, CultureInfo.InvariantCulture), DateTimeKind.Utc),
        };
    ...
    protected async Task<IActionResult> CreateJwtFromId(string uid, string charaIdent, string alias)
    {
        ...
        {
            new Claim(TeraClaimTypes.Uid, uid),
            new Claim(TeraClaimTypes.CharaIdent, charaIdent),
            new Claim(TeraClaimTypes.Alias, alias),
            new Claim(TeraClaimTypes.Expires, DateTime.UtcNow.AddHours(6).Ticks.ToString(CultureInfo.InvariantCulture)),
            new Claim(TeraClaimTypes.Continent, await _geoIPProvider.GetCountryFromIP(HttpAccessor))
        });
    ...
    protected async Task<bool> IsIdentBanned(TeraDbContext dbContext, string charaIdent)
    {
