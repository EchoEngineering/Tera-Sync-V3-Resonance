...
public class OAuthController : AuthControllerBase
{
    ...
    IHttpContextAccessor accessor, IDbContextFactory<TeraDbContext> teraDbContext,
    SecretKeyAuthenticatorService secretKeyAuthenticatorService,
    IConfigurationService<AuthServiceConfiguration> configuration,
    IDatabase redisDb, GeoIPService geoIPProvider)
        : base(logger, accessor, teraDbContext, secretKeyAuthenticatorService,
    ...
    public async Task<IActionResult> DiscordOAuthCallback([FromQuery] string code)
    {
        ...
        var teraUser = await dbContext.LodeStoneAuth.Include(u => u.User).SingleOrDefaultAsync(u => u.DiscordId == discordUserId);
        if (teraUser == default)
        {
            Logger.LogDebug("Failed to get Tera Sync V2 user for {session}, DiscordId: {id}", reqId, discordUserId);

            return BadRequest("Could not find a Tera Sync V2 user associated to this Discord account.");
        }
        ...
        {
            jwt = CreateJwt([
                new Claim(TeraClaimTypes.Uid, teraUser.User!.UID),
                new Claim(TeraClaimTypes.Expires, DateTime.UtcNow.AddDays(14).Ticks.ToString(CultureInfo.InvariantCulture)),
                new Claim(TeraClaimTypes.DiscordId, discordUserId.ToString()),
                new Claim(TeraClaimTypes.DiscordUser, discordUserName),
                new Claim(TeraClaimTypes.OAuthLoginToken, true.ToString())
            ]);
        }
        catch (Exception ex)
        {
    ...
    [HttpPost(TeraAuth.OAuth_GetUIDsBasedOnSecretKeys)]
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpPost(TeraAuth.OAuth_RenewOAuthToken)]
    public IActionResult RenewOAuthToken()
    {
        var claims = HttpContext.User.Claims.Where(c => c.Type != TeraClaimTypes.Expires).ToList();
        claims.Add(new Claim(TeraClaimTypes.Expires, DateTime.UtcNow.AddDays(14).Ticks.ToString(CultureInfo.InvariantCulture)));
        return Content(CreateJwt(claims).RawData);
    }

    [AllowAnonymous]
    [HttpGet(TeraAuth.OAuth_GetDiscordOAuthToken)]
    ...
    [AllowAnonymous]
    [HttpGet(TeraAuth.OAuth_GetDiscordOAuthEndpoint)]
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpGet(TeraAuth.OAuth_GetUIDs)]
    public async Task<Dictionary<string, string>> GetAvailableUIDs()
    {
        string primaryUid = HttpContext.User.Claims.Single(c => string.Equals(c.Type, TeraClaimTypes.Uid, StringComparison.Ordinal))!.Value;
        using var dbContext = await TeraDbContextFactory.CreateDbContextAsync();

        var teraUser = await dbContext.Auth.AsNoTracking().Include(u => u.User).FirstOrDefaultAsync(f => f.UserUID == primaryUid).ConfigureAwait(false);
        if (teraUser == null || teraUser.User == null) return [];
        var uid = teraUser.User.UID;
    ...
    [Authorize(Policy = "OAuthToken")]
    [HttpPost(TeraAuth.OAuth_CreateOAuth)]
    ...
    private async Task<IActionResult> AuthenticateOAuthInternal(TeraDbContext dbContext, string requestedUid, string charaIdent)
    {
        try
        {
            string primaryUid = HttpContext.User.Claims.Single(c => string.Equals(c.Type, TeraClaimTypes.Uid, StringComparison.Ordinal))!.Value;
