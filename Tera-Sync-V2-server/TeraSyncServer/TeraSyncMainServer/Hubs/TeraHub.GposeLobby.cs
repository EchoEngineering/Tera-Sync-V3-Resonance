...
public partial class TeraHub
{
    ...
    {
        _teraMetrics.IncGauge(MetricsAPI.GaugeGposeLobbyUsers);
        if (priorUsers.Count == 0)
            _teraMetrics.IncGauge(MetricsAPI.GaugeGposeLobbies);
    ...
    private async Task RemoveUserFromLobby(string lobbyId, List<string> priorUsers)
    {
        ...
        _teraMetrics.DecGauge(MetricsAPI.GaugeGposeLobbyUsers);

        if (priorUsers.Count == 1)
        {
            await _redis.RemoveAsync($"GposeLobby:{lobbyId}").ConfigureAwait(false);
            _teraMetrics.DecGauge(MetricsAPI.GaugeGposeLobbies);
        }
        else
        {
