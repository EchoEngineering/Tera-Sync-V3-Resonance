...
public partial class TeraHub
{
    public string UserCharaIdent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, TeraClaimTypes.CharaIdent, StringComparison.Ordinal))?.Value ?? throw new Exception("No Chara Ident in Claims");

    public string UserUID => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, TeraClaimTypes.Uid, StringComparison.Ordinal))?.Value ?? throw new Exception("No UID in Claims");

    public string Continent => Context.User?.Claims?.SingleOrDefault(c => string.Equals(c.Type, TeraClaimTypes.Continent, StringComparison.Ordinal))?.Value ?? "UNK";
    ...
    private async Task DeleteUser(User user)
    {
        ...
        _teraMetrics.IncCounter(MetricsAPI.CounterUsersRegisteredDeleted, 1);
    ...
    {
        _logger.LogCallInfo(TeraHubLogger.Args(dto));
        ...
        if (ownerHasLeft)
        {
            ...
            {
                _logger.LogCallInfo(TeraHubLogger.Args(dto, "Deleted"));
            ...
            else
            {
                ...
                {
                    _logger.LogCallInfo(TeraHubLogger.Args(dto, "Migrated", groupHasMigrated.Item2));
        ...
        _logger.LogCallInfo(TeraHubLogger.Args(dto, "Success"));
