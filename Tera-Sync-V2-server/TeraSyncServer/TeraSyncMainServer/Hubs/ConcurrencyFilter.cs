...
public sealed class ConcurrencyFilter : IHubFilter, IDisposable
{
    ...
    public ConcurrencyFilter(IConfigurationService<ServerConfiguration> config, TeraMetrics teraMetrics)
    {
        _config = config;
        ...
        _ = Task.Run(async () =>
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                if (stats != null)
                {
                    teraMetrics.SetGaugeTo(MetricsAPI.GaugeHubConcurrency, stats.CurrentAvailablePermits);
                    teraMetrics.SetGaugeTo(MetricsAPI.GaugeHubQueuedConcurrency, stats.CurrentQueuedCount);
                }
    ...
    {
        if (string.Equals(invocationContext.HubMethodName, nameof(TeraHub.CheckClientHealth), StringComparison.Ordinal))
        {
