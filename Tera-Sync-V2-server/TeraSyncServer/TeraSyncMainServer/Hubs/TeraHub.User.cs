...
public partial class TeraHub
{
    ...
    public async Task<List<OnlineUserIdentDto>> UserGetOnlinePairs(CensusDataDto? censusData)
    {
        ...
        _teraCensus.PublishStatistics(UserUID, censusData);
    ...
    {
        _logger.LogCallInfo(TeraHubLogger.Args(user));
    ...
    {
        _logger.LogCallInfo(TeraHubLogger.Args(dto.CharaData.FileReplacements.Count));
        ...
        foreach (var replacement in dto.CharaData.FileReplacements.SelectMany(p => p.Value))
        {
            ...
            {
                _logger.LogCallWarning(TeraHubLogger.Args("Invalid Data", "GamePaths", validGamePaths, string.Join(",", invalidPaths), "Hash", validHash, replacement.Hash, "FileSwap", validFileSwapPath, replacement.FileSwapPath));
                hadInvalidData = true;
        ...
        _logger.LogCallInfo(TeraHubLogger.Args(recipientUids.Count));
        ...
        _teraCensus.PublishStatistics(UserUID, dto.CensusDataDto);

        _teraMetrics.IncCounter(MetricsAPI.CounterUserPushData);
        _teraMetrics.IncCounter(MetricsAPI.CounterUserPushDataTo, recipientUids.Count);
    }
