...
{
    private readonly TeraMetrics metrics;
    private readonly ILogger<UserCleanupService> _logger;
    private readonly IDbContextFactory<TeraDbContext> _teraDbContextFactory;
    private readonly IConfigurationService<ServerConfiguration> _configuration;
    private CancellationTokenSource _cleanupCts;

    public UserCleanupService(TeraMetrics metrics, ILogger<UserCleanupService> logger, IDbContextFactory<TeraDbContext> teraDbContextFactory, IConfigurationService<ServerConfiguration> configuration)
    {
        this.metrics = metrics;
        _logger = logger;
        _teraDbContextFactory = teraDbContextFactory;
    ...
    private async Task CleanUp(CancellationToken ct)
    {
        ...
        {
            using (var dbContext = await _teraDbContextFactory.CreateDbContextAsync(ct).ConfigureAwait(false))
            {
    ...
    private async Task PurgeTempInvites(TeraDbContext dbContext)
    {
        try
        {
    ...
    private async Task PurgeUnusedAccounts(TeraDbContext dbContext)
    {
        try
        {
    ...
    private void CleanUpOutdatedLodestoneAuths(TeraDbContext dbContext)
    {
        try
        {
    ...
    public async Task PurgeUser(User user, TeraDbContext dbContext)
    {
