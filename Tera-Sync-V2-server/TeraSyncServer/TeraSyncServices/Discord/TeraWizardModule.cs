...
namespace TeraSyncV2Services.Discord;

public partial class TeraWizardModule : InteractionModuleBase
{
    private ILogger<TeraModule> _logger;
    private DiscordBotServices _botServices;
    private IConfigurationService<ServerConfiguration> _teraClientConfigurationService;
    private IConfigurationService<ServicesConfiguration> _teraServicesConfiguration;
    private IConnectionMultiplexer _connectionMultiplexer;
    private readonly IDbContextFactory<TeraDbContext> _dbContextFactory;
    private Random random = new();

    public TeraWizardModule(ILogger<TeraModule> logger, DiscordBotServices botServices,
        IConfigurationService<ServerConfiguration> teraClientConfigurationService,
        IConfigurationService<ServicesConfiguration> teraServicesConfiguration,
        IConnectionMultiplexer connectionMultiplexer, IDbContextFactory<TeraDbContext> dbContextFactory)
    {
        _logger = logger;
        _botServices = botServices;
        _teraClientConfigurationService = teraClientConfigurationService;
        _teraServicesConfiguration = teraServicesConfiguration;
    ...
    public async Task WizardCaptcha(bool init = false)
    {
        ...
        eb.WithTitle("Tera Sync V2 Bot Services Captcha");
    ...
    public async Task StartWizard(bool init = false)
    {
        ...
        using var teraDb = await GetDbContext().ConfigureAwait(false);
        bool hasAccount = await teraDb.LodeStoneAuth.AnyAsync(u => u.DiscordId == Context.User.Id && u.StartedAt == null).ConfigureAwait(false);

        if (init)
        {
            bool isBanned = await teraDb.BannedRegistrations.AnyAsync(u => u.DiscordIdOrLodestoneAuth == Context.User.Id.ToString()).ConfigureAwait(false);
...
#if !DEBUG
        bool isInAprilFoolsMode = _teraServicesConfiguration.GetValueOrDefault<ulong?>(nameof(ServicesConfiguration.DiscordRoleAprilFools2024), null) != null
        ...
        EmbedBuilder eb = new();
        eb.WithTitle("Welcome to the Tera Sync V2 Service Bot for this server");
        eb.WithDescription("Here is what you can do:" + Environment.NewLine + Environment.NewLine
            ...
            + (hasAccount ? string.Empty : ("- Register a new Tera Sync V2 Account press \"🌒 Register\"" + Environment.NewLine))
            ...
            + (!hasAccount ? string.Empty : (!isInAprilFoolsMode ? string.Empty : ("- Check your WorryCoin™ and TeraSyncV2Token© balance and add payment options" + Environment.NewLine)))
        ...
        else
        {
            ...
            {
                cb.WithButton("WorryCoin™ and TeraSyncV2Token© management", "wizard-fools", ButtonStyle.Primary, new Emoji("💲"));
            }
    ...
    private async Task<TeraDbContext> GetDbContext()
    {
    ...
    private async Task AddUserSelection(TeraDbContext teraDb, ComponentBuilder cb, string customId)
    {
        var discordId = Context.User.Id;
        var existingAuth = await teraDb.LodeStoneAuth.Include(u => u.User).SingleOrDefaultAsync(e => e.DiscordId == discordId).ConfigureAwait(false);
        if (existingAuth != null)
        {
            SelectMenuBuilder sb = new();
            sb.WithPlaceholder("Select a UID");
            sb.WithCustomId(customId);
            var existingUids = await teraDb.Auth.Include(u => u.User).Where(u => u.UserUID == existingAuth.User.UID || u.PrimaryUserUID == existingAuth.User.UID)
    ...
    private async Task AddGroupSelection(TeraDbContext db, ComponentBuilder cb, string customId)
    {
    ...
    private async Task<string> GenerateLodestoneAuth(ulong discordid, string hashedLodestoneId, TeraDbContext dbContext)
    {
