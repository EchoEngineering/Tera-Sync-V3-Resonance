...
public partial class TeraWizardModule
{
    ...
    public async Task ComponentDelete()
    {
        ...
        await AddUserSelection(teraDb, cb, "wizard-delete-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionDeleteAccount(string uid)
    {
        ...
        using var teraDb = await GetDbContext().ConfigureAwait(false);
        bool isPrimary = teraDb.Auth.Single(u => u.UserUID == uid).PrimaryUserUID == null;
        EmbedBuilder eb = new();
    ...
    public async Task ModalDeleteAccountConfirm(string uid, ConfirmDeletionModal modal)
    {
        ...
        try
        {
            ...
            {
                var maxGroupsByUser = _teraClientConfigurationService.GetValueOrDefault(nameof(ServerConfiguration.MaxGroupUserCount), 3);
