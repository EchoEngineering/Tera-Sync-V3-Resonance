...
internal class DiscordBot : IHostedService
{
    ...
    public DiscordBot(DiscordBotServices botServices, IServiceProvider services, IConfigurationService<ServicesConfiguration> configuration,
        IDbContextFactory<TeraDbContext> dbContextFactory,
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(token))
        {
            ...
            await _interactionModule.AddModuleAsync(typeof(TeraModule), _services).ConfigureAwait(false);
            await _interactionModule.AddModuleAsync(typeof(TeraWizardModule), _services).ConfigureAwait(false);
    ...
    private async Task OnUserJoined(SocketGuildUser arg)
    {
        try
        {
            using TeraDbContext dbContext = await _dbContextFactory.CreateDbContextAsync().ConfigureAwait(false);
    ...
    private async Task GenerateOrUpdateWizardMessage(SocketTextChannel channel, IUserMessage? prevMessage)
    {
        ...
        eb.WithTitle("Tera Sync V2 Services Bot Interaction Service");
        eb.WithDescription("Press \"Start\" to interact with this bot!" + Environment.NewLine + Environment.NewLine
            + "You can handle all of your Tera Sync V2 account needs in this server through the easy to use interactive bot prompt. Just follow the instructions!");
        eb.WithThumbnailUrl("https://raw.githubusercontent.com/kirin-xiv/TeraSyncV2/main/images/icon.png");
    ...
    {
        using TeraDbContext dbContext = await _dbContextFactory.CreateDbContextAsync(token).ConfigureAwait(false);
    ...
    private async Task CheckVanityForGroup(RestGuild restGuild, Dictionary<ulong, string> allowedRoleIds, TeraDbContext db, Group group, CancellationToken token)
    {
    ...
    private async Task CheckVanityForUser(RestGuild restGuild, Dictionary<ulong, string> allowedRoleIds, TeraDbContext db, LodeStoneAuth lodestoneAuth, CancellationToken token)
    {
    ...
    private async Task UpdateStatusAsync(CancellationToken token)
    {
        while (!token.IsCancellationRequested)
        {
            ...
            await _discordClient.SetActivityAsync(new Game("Tera Sync V2 for " + onlineUsers + " Users")).ConfigureAwait(false);
