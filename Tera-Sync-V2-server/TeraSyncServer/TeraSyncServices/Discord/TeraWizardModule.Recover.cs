...
public partial class TeraWizardModule
{
    ...
    public async Task ComponentRecover()
    {
        ...
        eb.WithDescription("In case you have lost your secret key you can recover it here." + Environment.NewLine + Environment.NewLine
            + "## ⚠️ **Once you recover your key, the previously used key will be invalidated. If you use Tera Sync V2 on multiple devices you will have to update the key everywhere you use it.** ⚠️" + Environment.NewLine + Environment.NewLine
        ...
        await AddUserSelection(teraDb, cb, "wizard-recover-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionRecovery(string uid)
    {
        ...
        await HandleRecovery(teraDb, eb, uid).ConfigureAwait(false);
        ComponentBuilder cb = new();
        AddHome(cb);
        await ModifyInteraction(eb, cb).ConfigureAwait(false);
    }

    private async Task HandleRecovery(TeraDbContext db, EmbedBuilder embed, string uid)
    {
        ...
        embed.WithDescription("This is your new private secret key. Do not share this private secret key with anyone. **If you lose it, it is irrevocably lost.**"
                              ...
                              + Environment.NewLine + Environment.NewLine
                              + "Enter this key in the Tera Sync V2 Service Settings and reconnect to the service.");
