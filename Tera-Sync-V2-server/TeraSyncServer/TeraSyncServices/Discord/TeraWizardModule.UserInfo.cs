...
public partial class TeraWizardModule
{
    ...
    public async Task ComponentUserinfo()
    {
        ...
        await AddUserSelection(teraDb, cb, "wizard-userinfo-select").ConfigureAwait(false);
        AddHome(cb);
    ...
    public async Task SelectionUserinfo(string uid)
    {
        ...
        await HandleUserInfo(eb, teraDb, uid).ConfigureAwait(false);
    ...
    private async Task HandleUserInfo(EmbedBuilder eb, TeraDbContext db, string uid)
    {
