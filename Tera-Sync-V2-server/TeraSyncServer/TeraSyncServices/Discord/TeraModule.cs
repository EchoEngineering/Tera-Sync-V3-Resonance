...
public class TeraModule : InteractionModuleBase
{
    private readonly ILogger<TeraModule> _logger;
    private readonly IServiceProvider _services;
    private readonly IConfigurationService<ServicesConfiguration> _teraServicesConfiguration;
    private readonly IConnectionMultiplexer _connectionMultiplexer;

    public TeraModule(ILogger<TeraModule> logger, IServiceProvider services,
        ...
        [Summary("uid", "User ID to the person to send the message to")] string? uid = null)
    {
        ...
        using var db = scope.ServiceProvider.GetService<TeraDbContext>();
        ...
        try
        {
            ...
            await c.PostAsJsonAsync(new Uri(_teraServicesConfiguration.GetValue<Uri>
            ...
            var discordChannelForMessages = _teraServicesConfiguration.GetValueOrDefault<ulong?>(nameof(ServicesConfiguration.DiscordChannelForMessages), null);
    ...
    private async Task<EmbedBuilder> HandleUserInfo(EmbedBuilder eb, ulong id, string? secondaryUserUid = null, ulong? optionalUser = null, string? uid = null)
    {
        ...
        using var scope = _services.CreateScope();
        await using var db = scope.ServiceProvider.GetRequiredService<TeraDbContext>();
        ...
        if (primaryUser == null)
        {
            ...
            eb.WithDescription("No Tera Sync V2 account was found associated to your Discord user");
            return eb;
        }
        ...
        else if ((optionalUser != null || uid != null) && isAdminCall)
        {
            ...
            {
                eb.WithTitle("No account");
                eb.WithDescription("The Discord user has no valid Tera Sync V2 account");
                return eb;
            }
