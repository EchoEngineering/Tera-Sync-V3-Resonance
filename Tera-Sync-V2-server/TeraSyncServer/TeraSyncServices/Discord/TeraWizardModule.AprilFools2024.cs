...
public partial class TeraWizardModule : InteractionModuleBase
{
    ...
    public async Task ComponentFools()
    {
        ...
        EmbedBuilder eb = new();
        eb.WithTitle("WorryCoin™ and TeraSyncV2Token© Balance");
    ...
    public async Task ComponentFoolsStart()
    {
        ...
        if (userIsInPermanentVanityRole)
        {
            ...
            eb.WithDescription("Thank you for participating in Teras 2024 April Fools event."
    ...
    public async Task ComponentFoolsWin(int number)
    {
        ...
        if (hasWon)
        {
            ...
            var user = await Context.Guild.GetUserAsync(Context.User.Id).ConfigureAwait(false);
            await user.AddRoleAsync(_teraServicesConfiguration.GetValue<ulong?>(nameof(ServicesConfiguration.DiscordRoleAprilFools2024)).Value).ConfigureAwait(false);
        }
        else
        {
